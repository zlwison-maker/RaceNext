...
test("coursePoints belong to Category and only the two accepted primary Categories contain them", () => {
...
test("accepted pointId values are semantic, stable and independent from displayOrder", () => {
  for (const category of [gongga, hk100]) {
...
test("list API does not expose coursePoints while Detail API does", () => {
  ...
  for (const editionId of ["kailas-gongga-100-2026", "hk100-2027"]) {
    ...
    ok(primary.coursePoints && primary.coursePoints.length > 0);
    equal(primary.coursePointDataStatus, "available");
  }
});
